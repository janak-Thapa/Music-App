...
const AlbumDetails = () => {
  ...
  const getAlbumDetails = async () => {
    try {
      const res = await axios.get(`https://saavn.dev/albums?id=${id}`); // Correct URL parameter
      const { data } = res.data; // Correct destructuring
      console.log(data);
      setAlbum(data);
      setSongs(data.songs);
      setImage(getImg(data.image)); // Correct property name for image
    } catch (error) {
